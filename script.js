...
.section,.trust{transition:opacity .7s ease,transform .7s ease}.section.reveal-pending{opacity:0;transform:translateY(28px)}.section.reveal-visible{opacity:1;transform:none}
...
.navcta,.btn.gold{transition:transform .2s ease,box-shadow .2s ease,background .2s ease}.navcta:hover,.btn.gold:hover{transform:translateY(-2px);box-shadow:0 12px 28px rgba(184,135,75,.26);background:#c09257;color:#fff}
.photo{width:max-content;max-width:100%;padding:10px 14px;border-left:2px solid var(--bronze);background:rgba(0,0,0,.2);backdrop-filter:blur(6px)}.trust{backdrop-filter:blur(14px)}
.visual.real-photo:before,.visual.real-photo:after,.project.real-photo:before{display:none!important}.visual.real-photo,.project.real-photo{background-size:cover!important;background-position:center!important;background-repeat:no-repeat!important}.project.real-photo{filter:saturate(.9) contrast(1.03)}
@media(max-width:720px){.hero{min-height:710px}.hero-bg{background-image:linear-gradient(180deg,rgba(0,0,0,.02) 0%,rgba(0,0,0,.2) 30%,rgba(8,8,8,.96) 71%,#080808 100%),url('assets/hero.svg')!important;background-position:62% 18%!important;background-size:auto 500px!important;background-repeat:no-repeat!important}.hero-copy{padding-top:255px}.hero-copy .eyebrow{font-size:9px}.hero-copy h1{letter-spacing:-.02em}.actions .btn{min-height:50px}.trust div{min-height:74px;display:flex;flex-direction:column;justify-content:center}.cards article:hover{transform:none}.dock{backdrop-filter:blur(16px);background:rgba(7,7,7,.95)}}
...
const realPhotos=['assets/project-1.svg','assets/project-2.svg','assets/hero.svg'];
document.querySelectorAll('.visual').forEach((el,i)=>{el.classList.add('real-photo');el.style.backgroundImage=`linear-gradient(rgba(0,0,0,.06),rgba(0,0,0,.10)),url('${realPhotos[i%realPhotos.length]}')`;});
document.querySelectorAll('.project').forEach((el,i)=>{el.classList.add('real-photo');el.style.backgroundImage=`linear-gradient(rgba(0,0,0,.08),rgba(0,0,0,.18)),url('${realPhotos[i%realPhotos.length]}')`;});
...
const header=document.querySelector('header');const updateHeader=()=>header?.classList.toggle('scrolled',window.scrollY>18);updateHeader();window.addEventListener('scroll',updateHeader,{passive:true});
const sections=document.querySelectorAll('.section');if('IntersectionObserver' in window){sections.forEach(s=>s.classList.add('reveal-pending'));const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.remove('reveal-pending');entry.target.classList.add('reveal-visible');observer.unobserve(entry.target);}}),{threshold:.08,rootMargin:'0px 0px -50px'});sections.forEach(s=>observer.observe(s));}else{sections.forEach(s=>s.classList.add('reveal-visible'));}
